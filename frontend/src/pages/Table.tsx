...
function Table() {
  ...
  const { data } = useQuery({
    queryKey: ["tables-data", dbName, tableName],
    queryFn: () => getTableData(dbName ?? "", tableName ?? ""),
    staleTime: 0,
    enabled: !!tableName,
  });
                ...
                <thead>
                  <tr className="bg-gray-100 text-left text-gray-700">
                    ...
                    {Object.keys(data[0]).map((key) => (
                      <th key={key} className="border px-4 py-2 w-auto whitespace-nowrap">{key}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {data.map((row: { [s: string]: unknown; } | ArrayLike<unknown>, index: Key | null | undefined) => (
                    <tr key={index} className="border-t hover:bg-gray-50">
                      ...
                      {Object.values(row).map((value, i) => (
                        <td key={i} className="border px-4 py-2 whitespace-nowrap">{String(value)}</td>
                      ))}
                    </tr>
                  ))}
                </tbody>
